...
Environment:
    ...
    ELEVENLABS_MUSIC_MODEL   optional override (ElevenLabs Music doesn't
                             ...
                             for forward-compat)
    MUSIC_DURATION_PAD_SEC   seconds of padding beyond voiceover end
                             (default: 2.0). Ignored when no voice-manifest.
...
def compute_duration_ms(artifacts_dir: Path) -> int:
    ...
    Priority: voice-manifest total + pad > plan nominal duration > MUSIC_MIN.
...
def main() -> None:
    ...
    composition_id = narration.get("composition_id") or "HnStory"
